"use client";
import { currentPlayStore, Isplaystore, MusicIdstore, playstore, roamingListStore } from "@/app/component/data";
...
export default function Page(){
    ...
    const handleplay = (songId: number) => {
      ...
      useEffect(() => {
        async function fetchData() {
          ...
          setCurrentPlay(roamingData.message); // Update the current play list
          // Assuming musicId should be set based on some logic, adjust if necessary
          if (roamingData.message.length > 0) {
            setMusicId(roamingData.message[0].id); // Set initial musicId
          }
        }
        fetchData();
      }, []);
    ...
    // Ensure useEffect runs only in client-side
    return(
        <div className="relative border  top-[80px] left-[280px] w-[1020px] h-[1120px]">
            <div className="mt-[32px] ml-[32px] h-[20px] mb-[32px] transition-colors text-base-content text-sm hover:text-primary" >
                ...
                <div className="absolute ml-[74px] hover:text-yellow-400 transition-colors"><a href="http://localhost:3000/trending">Trending</a></div>
                <div className="absolute ml-[144px] mt-[3.5px]"><IoIosArrowForward /></div>
                <div className="absolute ml-[168px] text-yellow-400">Suno AI Songs</div>  
            </div>
            <div className="ml-[32px] mb-[22px] h-[25px] text-2xl font-semibold tracking-tight ">Trending</div>
            <div className="ml-[32px] w-[315px] mb-[25px] h-[40px] flex bg-gray-100 border border-transparent rounded-[8px]">
                <div className="mt-[3px]  pl-[15px] pt-[4px] text-[15px] h-[33px] w-[49px] ml-[4px]  "><a href="http://localhost:3000/trending">All</a></div>
                <div className=" mt-[3px] border rounded-[8px]  pl-[12px] pt-[5.5px] text-[14.5px] h-[33px] w-[130px] ml-[4px] bg-yellow-400"><a href="http://localhost:3000/trending/suno-ai-songs">Suno AI Songs</a></div>
                <div className="mt-[3px]  rounded-[8px] pl-[11.5px] pt-[5.5px] text-[14.5px] h-[33px] w-[130px] ml-[4px] "><a href="http://localhost:3000/trending/udio-ai-songs">Udio AI Songs</a></div>
            </div>
            ...
            {roamingData.slice(0,8).map((item,index) => (
